...
class AgentEngines(_api_module.BaseModule):
    ...
    def check_query_job(
        ...
        *,
        ...
        try:
            raw_response = self._api_client.request("get", name, None, None)
            print(f"DEBUG CheckQueryJob raw_response type: {type(raw_response)}")
            print(f"DEBUG CheckQueryJob raw_response: {raw_response}")
            print(f"DEBUG CheckQueryJob raw_response dir: {dir(raw_response)}")

            if hasattr(raw_response, "body"):
                import json

                print(
                    f"DEBUG CheckQueryJob raw_response.body type: {type(raw_response.body)}"
                )
                print(f"DEBUG CheckQueryJob raw_response.body: {raw_response.body}")
                operation = (
            ...
            print(f"DEBUG CheckQueryJob operation type: {type(operation)}")
            print(f"DEBUG CheckQueryJob operation: {operation}")
        except Exception as e:
            raise ValueError(f"Failed to retrieve operation {name}: {e}")
    ...
    def run_query_job(
        ...
        *,
        ...
        """Runs a query job on an agent engine asynchronously.
        ...
        Args:
            ...
            config (RunQueryJobAgentEngineConfigOrDict):
                ...
                the following fields:
                  ...
                  - gcs_bucket: The GCS bucket to use for the query.
        """
        from google.cloud import storage  # type: ignore[attr-defined]
